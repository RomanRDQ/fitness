(() => {
    ...
    let _slideToggle = (target, duration = 500) => {
        if (!target.classList.contains("_slide")) {
            ...
            if ("none" === window.getComputedStyle(target).display) return _slideDown(target, duration); else return _slideUp(target, duration);
        }
    };
    ...
    function select_item(select) {
        ...
        if ("input" == select_type) select_type_content = '<div class="select__value icon-select-arrow"><input autocomplete="off" type="text" name="form[]" value="' + select_selected_text + '" data-error="Ошибка" data-value="' + select_selected_text + '" class="select__input"></div>'; else select_type_content = '<div class="select__value icon-select-arrow"><span>' + select_selected_text + "</span></div>";
    ...
    function initSliders() {
        ...
        if (document.querySelector(".products-slider")) {
            new core(".products-slider__item", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        if (document.querySelector(".images-product")) {
            let imagesSubSlider = new core(".images-product__subslider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
            ...
            new core(".images-product__mainslider", {
    ...
    (function(factory) {
        if ("function" === typeof define && define.amd) define([], factory); else if ("object" === typeof exports) module.exports = factory(); else window.noUiSlider = factory();
    })((function() {
        "use strict";
        ...
        function handleEntryPoint(index, value, that) {
            ...
            } else that.xSteps.push(isNaN(value[1]) ? false : value[1]);
        ...
        function Spectrum(entry, snap, singleStep) {
            ...
            if (ordered.length && "object" === typeof ordered[0][0]) ordered.sort((function(a, b) {
                return a[0][0] - b[0][0];
        ...
        function testSnap(parsed, entry) {
            parsed.snap = entry;
            if ("boolean" !== typeof entry) throw new Error("noUiSlider (" + VERSION + "): 'snap' option must be a boolean.");
        }
        function testAnimate(parsed, entry) {
            parsed.animate = entry;
            if ("boolean" !== typeof entry) throw new Error("noUiSlider (" + VERSION + "): 'animate' option must be a boolean.");
        }
        function testAnimationDuration(parsed, entry) {
            parsed.animationDuration = entry;
            if ("number" !== typeof entry) throw new Error("noUiSlider (" + VERSION + "): 'animationDuration' option must be a number.");
        }
        ...
        function testPadding(parsed, entry) {
            ...
            for (index = 0; index < parsed.spectrum.xNumSteps.length - 1; index++) if (parsed.padding[0][index] < 0 || parsed.padding[1][index] < 0) throw new Error("noUiSlider (" + VERSION + "): 'padding' option must be a positive number(s).");
            var totalPadding = entry[0] + entry[1];
            var firstValue = parsed.spectrum.xVal[0];
            var lastValue = parsed.spectrum.xVal[parsed.spectrum.xVal.length - 1];
            if (totalPadding / (lastValue - firstValue) > 1) throw new Error("noUiSlider (" + VERSION + "): 'padding' option must not exceed 100% of the range.");
        }
        ...
        function testTooltips(parsed, entry) {
            ...
            if (true === entry) {
                parsed.tooltips = [];
                for (var i = 0; i < parsed.handles; i++) parsed.tooltips.push(true);
            } else {
        ...
        function testKeyboardSupport(parsed, entry) {
            ...
            if ("boolean" !== typeof entry) throw new Error("noUiSlider (" + VERSION + "): 'keyboardSupport' option must be a boolean.");
        }
        function testDocumentElement(parsed, entry) {
            parsed.documentElement = entry;
        }
        function testCssPrefix(parsed, entry) {
            if ("string" !== typeof entry && false !== entry) throw new Error("noUiSlider (" + VERSION + "): 'cssPrefix' must be a string or `false`.");
        ...
        function scope(target, options, originalOptions) {
            ...
            function addTooltip(handle, handleNumber) {
                if (!options.tooltips[handleNumber]) return false;
            ...
            function getGroup(mode, values, stepped) {
                ...
                if ("positions" === mode) return values.map((function(value) {
                    return scope_Spectrum.fromStepping(stepped ? scope_Spectrum.getStep(value) : value);
                }));
                if ("values" === mode) {
                    if (stepped) return values.map((function(value) {
            ...
            function generateSpread(density, mode, group) {
                ...
                group.forEach((function(current, index) {
                    ...
                    var realSteps;
                    var stepSize;
                    var isSteps = "steps" === mode;
                    if (isSteps) step = scope_Spectrum.xNumSteps[index];
                    if (!step) step = high - low;
                    if (false === low || void 0 === high) return;
            ...
            function eventTap(event) {
                if (!event.buttons && !event.touches) return false;
                event.stopPropagation();
                var proposal = calcPointToPercentage(event.calcPoint);
                var handleNumber = getClosestHandle(proposal);
                if (false === handleNumber) return false;
                ...
                if (options.events.snap) eventStart(event, {
            ...
            function removeEvent(namespacedEvent) {
                ...
                Object.keys(scope_Events).forEach((function(bind) {
                    ...
                    if ((!event || event === tEvent) && (!namespace || namespace === tNamespace)) delete scope_Events[bind];
                }));
            }
            ...
            function moveHandles(upward, proposal, locations, handleNumbers) {
                ...
                if (state) handleNumbers.forEach((function(handleNumber) {
                    fireEvent("update", handleNumber);
                    fireEvent("slide", handleNumber);
                }));
            }
    ...
    let menuPageBurgerCatalog = document.querySelector(".menu-page__burger_catalog");
    let menuPageBodyCatalog = document.querySelector(".menu-page__body_catalog");
    menuPageBurgerCatalog.addEventListener("click", (function(e) {
        menuPageBurgerCatalog.classList.toggle("_active");
        _slideToggle(menuPageBodyCatalog);
    }));
    let menuPageBurgerProduct = document.querySelector(".menu-page__burger_product");
    let menuPageBodyProduct = document.querySelector(".menu-page__body_product");
    menuPageBurgerProduct.addEventListener("click", (function(e) {
        menuPageBurgerProduct.classList.toggle("_active");
        _slideToggle(menuPageBodyProduct);
    }));
    let menuPageBurgerCheckout = document.querySelector(".menu-page__burger_checkout");
    let menuPageBodyCheckout = document.querySelector(".menu-page__body_checkout");
    menuPageBurgerCheckout.addEventListener("click", (function(e) {
        menuPageBurgerCheckout.classList.toggle("_active");
        _slideToggle(menuPageBodyCheckout);
    }));
    ...
    let searchSelectCatalog = document.querySelector(".search-page__title_catalog");
    let categoriesSearchCatalog = document.querySelector(".categories-search_catalog");
    searchSelectCatalog.addEventListener("click", (function(e) {
        searchSelectCatalog.classList.toggle("_active");
        _slideToggle(categoriesSearchCatalog);
    }));
    let searchSelectProduct = document.querySelector(".search-page__title_product");
    let categoriesSearchProduct = document.querySelector(".categories-search_product");
    searchSelectProduct.addEventListener("click", (function(e) {
        searchSelectProduct.classList.toggle("_active");
        _slideToggle(categoriesSearchProduct);
    }));
    let searchSelectCheckout = document.querySelector(".search-page__title_checkout");
    let categoriesSearchCheckout = document.querySelector(".categories-search_checkout");
    searchSelectCheckout.addEventListener("click", (function(e) {
        searchSelectCheckout.classList.toggle("_active");
        _slideToggle(categoriesSearchCheckout);
    }));
    ...
    for (let index = 0; index < checkboxCategories.length; index++) {
        ...
        checkboxCategory.addEventListener("change", (function(e) {
            ...
            } else searchSelect.classList.remove("_categories");
        }));
    }
